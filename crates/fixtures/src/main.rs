//! `gen-fixtures`: generate the ledger fixtures and check them against the
//! pinned hashes in the committed manifest
//!
//! Crate convention: never iterate a std `HashMap`. Output order must depend only
//! on the inputs, never on a per-process hash seed, so this crate uses `BTreeMap`
//! or sorted vecs everywhere output order could otherwise leak into a file.
...
// tested via `just check` in CI so no need to include main in
// code coverage
#[cfg_attr(coverage_nightly, coverage(off))]
fn main() -> ExitCode {
    let args = Args::parse();
    // Locate the files from the crate's own path and not the working directory,
...
///
/// This scaffolds only a `main.ledger`: a header comment recording the inputs
/// plus an include skeleton, one journal per calendar year in the window
fn generate(manifest: &Manifest) -> Result<BTreeMap<String, String>, Box<dyn Error>> {
    ...
    for year in years {
        let _ = writeln!(body, "include {year}.ledger");
    }
...
mod tests {
    ...
    fn generated_ledger_has_header_and_year_includes() {
        ...
        assert!(body.contains("include 2015.ledger"));
        assert!(body.contains("include 2024.ledger"));
    }
