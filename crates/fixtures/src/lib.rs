...
/// journal is emitted alongside as a header-only placeholder.
///
/// # Errors
...
pub fn generate(manifest: &Manifest) -> Result<BTreeMap<String, String>, Box<dyn Error>> {
    ...
    for year in years {
        ...
        // A header-only placeholder until the banking emitter fills it. Ledger
        // reads a comment-only file as an empty journal, so the includes all
        // resolve and `ledger balance` on the skeleton is clean
        let mut year_body = String::new();
        let _ = writeln!(year_body, "; firepath transactions for {year}");
        year_body.push_str("; generated by `just gen-fixtures`, do not edit by hand\n");
        files.insert(name, year_body);
    }
...
mod tests {
    ...
    fn every_include_resolves_to_a_generated_file() {
        ...
        // generated, so `ledger` can read the whole fixture, and each placeholder
        // is a comment-only journal
